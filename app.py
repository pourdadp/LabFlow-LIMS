...
def init_db():
    ...
    for col in ['additional_info', 'confidential_info']:
        try: c.execute(f"ALTER TABLE samples ADD COLUMN {col} TEXT")
        except: pass
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS test_definitions
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  ...
                  parameters TEXT)''')
    try: c.execute("ALTER TABLE test_definitions ADD COLUMN parameters TEXT")
    except: pass
...
def add_notification(username, message, link=''):
    conn = sqlite3.connect(DATABASE, timeout=30)
    c = conn.cursor()
    c.execute("INSERT INTO notifications (username, message, link, created_time) VALUES (?,?,?,?)",
...
def get_test_map():
    ...
    conn.close()
    test_map = {}
    for name, dept, params in rows:
        test_map.setdefault(dept, []).append((name, params or ''))
    return test_map
...
def register():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DATABASE, timeout=30)
        c = conn.cursor()
        c.execute("INSERT INTO samples (sample_id, type, source, description, additional_info, confidential_info, confidential, status, submitted_by, received_by, receiving_department, created_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                  (sample_id, request.form['type'], request.form['source'], request.form['description'],
                   ...
                   request.form['submitted_by'], request.form['received_by'], request.form['receiving_department'], today))
        for tt in request.form.getlist('test_type[]'):
            if tt:
                c.execute("INSERT INTO sample_tests (sample_id, test_type, department) VALUES (?,?,?)",
                          (sample_id, tt, request.form['receiving_department']))
        conn.commit()
        conn.close()
        log_action(sample_id, session['user'], 'Sample registered')
        add_notification(request.form['received_by'], f'New sample {sample_id} assigned to you.', f'/sample/{sample_id}')
        return redirect(url_for('dashboard'))
...
def receive_sample(sample_id):
    ...
    sample = c.fetchone()
    if not sample or (session['role'] != 'admin' and session['user'] != sample[8]):
        conn.close()
        return "Access denied", 403
    c.execute("UPDATE samples SET status='Pending' WHERE sample_id=?", (sample_id,))
    conn.commit()
    conn.close()
    log_action(sample_id, session['user'], 'Sample received')
    add_notification(sample[7], f'Sample {sample_id} has been received.', f'/sample/{sample_id}')
...
def my_samples():
    ...
    params = []
    if session['role'] != 'admin':
        query += " AND (submitted_by=? OR received_by=? OR confidential=0)"
        params.extend([session['user'], session['user']])
    if search:
        query += " AND (sample_id LIKE ? OR description LIKE ? OR source LIKE ?)"
        params.extend([f'%{search}%'] * 3)
    if status_filter:
        query += " AND status=?"
        params.append(status_filter)
    if dept_filter:
        query += " AND receiving_department=?"
...
def sample_detail(sample_id):
    ...
    sample = c.fetchone()
    if not sample or (sample[5] and session['role'] != 'admin' and session['user'] not in [sample[7], sample[8]]):
        conn.close()
        return "Access denied", 403
        ...
        if 'image' in request.files:
            ...
            if file and file.filename:
                ...
                if ext in {'jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'}:
                    safe_fname = secure_filename(f"{sample_id}_{test_type}_rep{replicate}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{ext}")
    ...
    return render_template_string(SAMPLE_DETAIL_HTML, sample=sample, requested_tests=requested_tests,
                                  grouped=grouped, dept_tests=dept_tests)
...
def print_sample(sample_id):
    ...
    results_raw = c.fetchall()
    conn.close()
    return render_template_string(PRINT_SAMPLE_HTML, sample=sample, requested_tests=requested_tests, results_raw=results_raw)
...
def update_status(sample_id):
    ...
    conn = sqlite3.connect(DATABASE, timeout=30)
    c = conn.cursor()
    c.execute("UPDATE samples SET status=? WHERE sample_id=?", (request.form['status'], sample_id))
    conn.commit()
    conn.close()
...
def request_password_change():
    ...
    conn = sqlite3.connect(DATABASE, timeout=30)
    c = conn.cursor()
    c.execute("UPDATE password_change_requests SET status='expired' WHERE username=? AND status='pending'", (username,))
    c.execute("INSERT INTO password_change_requests (username, request_time, status) VALUES (?,?,?)",
              (username, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'pending'))
    conn.commit()
    conn.close()
...
<div class="row"><div class="col-md-6 mb-3"><label>Submitted By</label>
...
{% endif %}</div>
<div class="col-md-6 mb-3"><label>Receiving Department</label><select name="receiving_department" id="receiving_department" class="form-select" required onchange="updateReceivers();updateTestTypes();"><option value="">-- Select --</option>{% for d in receive_depts %}<option value="{{ d }}">{{ d }}</option>{% endfor %}</select></div></div>
<div class="mb-3"><label>Receiver (person)</label><select name="received_by" id="received_by" class="form-select" required disabled><option value="">-- First select department --</option></select></div>
...
<h5>Requested Tests</h5><div id="testList"><div class="row mb-2 test-row"><div class="col-md-10"><select name="test_type[]" class="form-select test-type-select"><option value="">-- Select test --</option></select></div><div class="col-md-2"><button type="button" class="btn btn-danger btn-sm" onclick="this.closest('.test-row').remove()">✕</button></div></div></div>
<button type="button" class="btn btn-secondary btn-sm" onclick="addTestRow()">+ Add Test</button><hr>
...
function updateReceivers(){const dept=document.getElementById('receiving_department').value;const receiverSelect=document.getElementById('received_by');receiverSelect.innerHTML='';if(dept&&deptUsers[dept]){receiverSelect.disabled=false;deptUsers[dept].forEach(u=>{const opt=document.createElement('option');opt.value=u[0];opt.textContent=u[1]+' ('+u[0]+')';receiverSelect.appendChild(opt);});}else{receiverSelect.disabled=true;const opt=document.createElement('option');opt.value='';opt.textContent='-- First select department --';receiverSelect.appendChild(opt);}}
function updateTestTypes(){const dept=document.getElementById('receiving_department').value;const tests=(testMap[dept]||[]).map(t=>typeof t==='string'?t:t[0]);document.querySelectorAll('.test-type-select').forEach(select=>{select.innerHTML='<option value="">-- Select test --</option>';tests.forEach(t=>{const opt=document.createElement('option');opt.value=t;opt.textContent=t;select.appendChild(opt);});});}
function addTestRow(){const row=document.querySelector('.test-row').cloneNode(true);row.querySelector('.test-type-select').innerHTML=document.querySelector('.test-type-select').innerHTML;document.getElementById('testList').appendChild(row);}
...
<p><strong>Receiving Department:</strong> {{ sample[9] }}</p><p><strong>Date:</strong> {{ sample[10] }}</p><p><strong>Confidential:</strong> {{ 'Yes' if sample[5] else 'No' }}</p></div>
...
<div class="mt-3"><form method="post" action="/receive_sample/{{ sample[1] }}"><input type="hidden" name="_csrf_token" value="{{ csrf_token() }}"><button type="submit" class="btn btn-info">Confirm Receipt</button></form></div>{% endif %}
<h4 class="mt-4">Requested Tests</h4><table class="table"><thead><tr><th>Test Type</th><th>Department</th></tr></thead>
...
<h4 class="mt-4">Update Status</h4><form method="post" action="/update_status/{{ sample[1] }}"><input type="hidden" name="_csrf_token" value="{{ csrf_token() }}">
...
<div class="container" id="printArea"><h2>LabFlow LIMS - Sample Report</h2><hr>
...
<hr><p>Printed by: {{ session['full_name'] }} on {{ now }}</p></div>
